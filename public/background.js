const canvas = document.querySelector('#hyperdrive')
	...
	function run() {
		...
		const bubbleArray = () => {
			...
			return {
				...
				update() {
					...
					head = tail = 0
					while (head < count) {
						if (items[head].update() === false) { head += 1 } else {
							if (tail < head) {
				...
				callEach(name) { let i = 0; while (i < count) { if (items[i++][name]() === true) { break } } },
				each(cb) { let i = 0; while (i < count) { if (cb(items[i], i++) === true) { break } } },
		...
		const doFor = (count, cb) => { let i = 0; while (i < count && cb(i++) !== true); } // the ; after while loop is important don't remove

		const sCurve = (v, p) => (2 / (1 + Math.pow(p, -v))) - 1
		const randHSLA = (h, h1, s = 0, s1 = 0, l = 0, l1 = 0, a = 1, a1 = 1) => { return `hsla(${randI(h, h1) % 360},${randI(s, s1)}%,${randI(l, l1)}%,${rand(a, a1)})` }
